      ...
      <div className="bg-gradient-to-r from-rose-500 to-pink-400 text-white px-4 py-5 relative overflow-hidden">
        ...
        <div className="max-w-xl mx-auto flex justify-between items-center relative">
          ...
          <div className="flex gap-2">
            <Link href="/" className="text-sm bg-white/25 px-3 py-1.5 rounded-full">🔍 探す</Link>
            {user && (
              <button onClick={handleLogout} className="text-sm bg-white/25 px-3 py-1.5 rounded-full">ログアウト</button>
            )}
          </div>
        </div>
      </div>
      <div className="max-w-xl mx-auto px-4 py-4 pb-16">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link href="/login" className="bg-rose-500 text-white px-6 py-3 rounded-xl text-sm font-semibold">ログインする</Link>
          </div>
        ) : shops.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-4xl mb-3">♡</p>
            <p className="text-gray-500 mb-4">まだお気に入りがありません</p>
            <Link href="/" className="bg-rose-500 text-white px-6 py-3 rounded-xl text-sm font-semibold">店舗を探す</Link>
          </div>
        ) : (
          <>
            ...
            {shops.map(shop => (
              <Link key={shop.id} href={`/shops/${shop.id}`}>
                <div className="bg-white border border-rose-100 rounded-xl p-4 mb-3 hover:shadow-md transition">
