  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-green-700 text-white px-4 py-5">
        <button onClick={() => router.back()} className="text-sm opacity-80 mb-2 block">← 店舗詳細に戻る</button>
        <h1 className="text-xl font-bold">口コミを書く</h1>
        <p className="text-sm opacity-85">{shop.name} · {shop.area}</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="料理の味、雰囲気、アクセスなど自由に書いてください（5文字以上）"
              rows={5}
              required
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-400 resize-none"
            />
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-green-700 text-white py-3 rounded-xl font-semibold text-sm disabled:opacity-50"
          >
